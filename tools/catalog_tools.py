...
def catalog_record_checksum(catalog):
    payload = json.dumps({'records': catalog['records']}, ensure_ascii=False, sort_keys=True, separators=(',', ':')).encode('utf-8')
...
def generate_amalgam_index(catalog):
    ...
    return sorted(entries, key=lambda item: (
        item['requiredDiscipline'], item['requiredLevel'], item['recordDiscipline'],
        item['recordLevel'], item['recordName']['en']
    ))
